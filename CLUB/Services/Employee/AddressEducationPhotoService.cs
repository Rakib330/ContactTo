...
namespace CLUB.Services.Employee
{
    public class AddressEducationPhotoService : IEmployeeInfoService
    {
        ...
        public AddressEducationPhotoService(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            return await _context.addresses.Include(x=> x.division).Include(x=>x.district).Include(x=>x.thana).Where(x => x.employeeId == empId && x.type == type).FirstOrDefaultAsync();
        }
